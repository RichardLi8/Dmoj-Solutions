...
public class HelloWorld {
    ...
    static void comb(int pos){
        lft [pos] = Math.max(lft [2*pos], Math.max(sum [2*pos] + lft [2*pos+1], sum [2*pos]));
        rit [pos] = Math.max(rit [2*pos+1], Math.max(sum [2*pos+1] + rit [2*pos], sum [2*pos+1]));
    ...
    static data query(int l, int r, int ql, int qr, int pos){
        ...
        if(qr <= mid){
            ...
            cur.lft = Math.max(lc.lft, Math.max(lc.sum + rc.lft, lc.sum));
            cur.rit = Math.max(rc.rit, Math.max(rc.sum + lc.rit, rc.sum));
